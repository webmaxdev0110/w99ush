<?php
	...
	****************************************/
	...
	if ( isset( $_POST['submit'] ) ) {
		...
		$result = $wpdb->get_results("SELECT * FROM xiblox_destination_info WHERE connection_name = '$connection_name'");
		if ($result) {
			$wpdb->query( "UPDATE xiblox_destination_info SET connection_name = '$connection_name', destination_url = '$destination_url', db_host = '$db_host', db_name = '$db_name', db_user = '$db_user', db_password = '$db_password', db_prefix = '$db_prefix', destination_path = '$destination_path', ftp_host = '$ftp_host', ftp_user = '$ftp_user', ftp_password = '$ftp_password', ftp_ssl = '$ftp_ssl' WHERE id = " . $number );
		}
		else {
			$wpdb->query( "INSERT INTO xiblox_destination_info(connection_name, destination_url, db_host, db_name, db_user, db_password, db_prefix, destination_path, ftp_host, ftp_user, ftp_password, ftp_ssl)
			VALUES('$connection_name', '$destination_url', '$db_host', '$db_name', '$db_user', '$db_password', '$db_prefix', '$destination_path', '$ftp_host', '$ftp_user', '$ftp_password', '$ftp_ssl')");
		}
